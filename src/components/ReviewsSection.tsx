...
import {
  HOST_PUBLISHED_REVIEWS_LS_KEY,
  HOST_REVIEWS_UPDATED_EVENT,
  readHostReviewsSorted,
...
type FeaturedSlot = ReviewEntry & { cardKey: string }

function Stars({ rating, size = 'md' }: { rating: 1 | 2 | 3 | 4 | 5; size?: StarSize }) {
  const dim =
    size === 'lg'
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <Stars rating={data.stars} size="lg" />
        <span className="rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-blue-700 ring-1 ring-blue-200/80">
          {data.stars === 5 ? '5/5' : data.stars === 4 ? '4/5' : `${data.stars}/5`}
        </span>
      </div>
      <p className="mt-4 text-base font-bold leading-snug text-zinc-900 sm:text-lg">&ldquo;{data.quote}&rdquo;</p>
      <hr className="my-4 border-zinc-200/80" />
      <div>
        <p className="font-semibold text-zinc-900">{data.name}</p>
        <p className="mt-1 text-sm text-zinc-600">{data.role}</p>
      </div>
    </motion.article>
  )
}
...
function CompactReviewCard({ review }: { review: FeaturedSlot }) {
  ...
  return (
    ...
    >
      <Stars rating={data.stars} size="sm" />
      <p className="mt-2.5 text-[13px] font-semibold leading-snug text-zinc-900 sm:text-sm">{data.quote}</p>
...
function hostToFeaturedSlot(h: StoredHostReview): FeaturedSlot {
  return {
    cardKey: h.id,
    stars: h.stars,
    quote: h.quote,
...
export function ReviewsSection() {
  ...
  useEffect(() => {
    const fn = () => setHostVersion((v) => v + 1)
    const onStorage = (e: StorageEvent) => {
      if (e.key === HOST_PUBLISHED_REVIEWS_LS_KEY) fn()
    }
    window.addEventListener(HOST_REVIEWS_UPDATED_EVENT, fn)
    window.addEventListener('storage', onStorage)
    return () => {
      window.removeEventListener(HOST_REVIEWS_UPDATED_EVENT, fn)
      window.removeEventListener('storage', onStorage)
    }
  }, [])

  const hostReviews = useMemo(() => readHostReviewsSorted(), [hostVersion])

  const featuredDisplay = useMemo((): FeaturedSlot[] => {
    const fromHosts = hostReviews.slice(0, 3).map(hostToFeaturedSlot)
    const need = 3 - fromHosts.length
    const fromStatic = t.featured.slice(0, Math.max(0, need)).map((r, i) => ({
      ...r,
      cardKey: `static-feat-${i}-${r.name}`,
    }))
    return [...fromHosts, ...fromStatic].slice(0, 3)
  }, [hostReviews, t.featured])

  const moreMerged = useMemo(() => {
    const hostTail = hostReviews.slice(3).map((h) => hostToFeaturedSlot(h))
    const staticMore = t.more.map((r, i) => ({ ...r, cardKey: `static-more-${i}-${r.name}` }))
    return [...hostTail, ...staticMore]
  }, [hostReviews, t.more])

  const loopedMore = useMemo(() => [...moreMerged, ...moreMerged], [moreMerged])
      ...
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mt-10 grid gap-4 rounded-2xl border border-zinc-200/70 bg-white/70 p-3 shadow-pm-sm sm:mt-12 sm:grid-cols-[minmax(0,1fr)_260px] sm:p-4 lg:grid-cols-[minmax(0,1fr)_290px]">
          <div className="grid grid-cols-1 gap-3 sm:gap-4 lg:grid-cols-3">
            {featuredDisplay.map((review) => (
              <FeaturedReviewCard key={review.cardKey} review={review} />
            ))}
          </div>
          <aside className="rounded-xl border border-zinc-200/80 bg-white p-2.5 sm:p-3">
            ...
            >
              {loopedMore.map((review, i) => (
                <CompactReviewCard key={`more-${review.cardKey}-${i}`} review={review} />
              ))}
            </div>
          </aside>
        </div>
